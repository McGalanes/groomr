...
import com.github.mcgalanes.groomr.core.ui.MyApplicationTheme
...
@Composable
fun FooScreen(modifier: Modifier = Modifier, viewModel: FooViewModel = hiltViewModel()) {
    val items by viewModel.uiState.collectAsStateWithLifecycle()
    if (items is Success) {
        FooScreen(
            items = (items as Success).data,
            onSave = { name -> viewModel.addFoo(name) },
            modifier = modifier
        )
    }
}
